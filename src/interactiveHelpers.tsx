...
export function showSetupDialog<T = void>(root: Root, renderer: (done: (result: T) => void) => React.ReactNode, options?: {
  ...
  // Release cli.tsx early stdin capture before Ink mounts. If both register
  // `readable` on stdin, whichever runs first drains the buffer — earlyInput
  // discards arrow-key escape sequences, so ThemePicker/onboarding lists stay
  // dead until this runs (Bun + WSL/Cursor is especially sensitive).
