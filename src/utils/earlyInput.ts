...
export function stopCapturingEarlyInput(): void {
  ...
  // Return to cooked mode before Ink's App enables raw mode again. Leaving
  // raw mode on after removing our listener breaks stdin on some runtimes
  // (Bun + WSL/Cursor): keystrokes never reach Ink's readable handler.
  try {
    if (process.stdin.isTTY && process.stdin.setRawMode) {
      process.stdin.setRawMode(false)
    }
  } catch {
    // ignore — non-TTY or already closed
  }
}
